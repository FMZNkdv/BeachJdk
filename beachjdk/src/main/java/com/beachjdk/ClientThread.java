            ...
            while (true) {
                ...
                if (header.length >= 7) {
                    if (length == data.length) {
                        System.out.println("[C] " + packetID + " received.");
                        try {
                            ...
                            if (PacketsFactory.availablePackets.containsKey(packetID)) {
                                ...
                                System.out.println("[C] " + packetID + " not handled.");
                            }
                        } catch (Exception e) {
                            System.out.println("[C] Error while decrypting / handling " + packetID);
                            e.printStackTrace();
                        }
                    } else {
                        System.out.println("[C] Incorrect Length for packet " + packetID + " (header length: " 
                            + length + ", data length: " + data.length + ").");
                    }
                } else {
                    System.out.println("[C]] Received an invalid packet from client.");
                    client.close();
                    break;
                }
            }
